...
func TestQuantityParse(t *testing.T) {
	tests := []struct {
		...
		{"10 m", 10, Unit(Meter)},
		{"10m", 10, Unit(Meter)},
		{"  1.5  km  ", 1.5, Unit(Kilometer)},
		{"100 kg", 100, Unit(Kilogram)},
		{"-5 °C", -5, Unit(Celsius)},
		{"+3.2 s", 3.2, Unit(Second)},
		{"1,024 mm", 1024, Unit(Millimeter)},
		{"1_024 mm", 1024, Unit(Millimeter)},
		{"10 oz t", 10, Unit(TroyOunce)},
		{"5 N", 5, Unit(ForceUnit.Key())},
		{"5 kg·m·s^-2", 5, Unit(ForceUnit.Key())},
		{"5 kg·m·s⁻²", 5, Unit(ForceUnit.Key())},
		{"60 m/s", 60, Unit(SpeedUnit.Key())},
