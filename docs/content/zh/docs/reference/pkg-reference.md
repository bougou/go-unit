...
| `pkg/u/derivedunit.go` / `pkg/u/derivedquantity.go` | 导出单位与导出量 |
| `pkg/u/none_unit.go` | 无量纲 |
| `pkg/u/prefix*.go` / `pkg/u/roundmethod.go` | 数值词头 |
| `pkg/u/comma.go` | 千分位格式化 |
| `pkg/u/byte_consts.go` | 字节常量 |

## 主要类型

`Dimension`、`DerivedDimension`、`Unit`、`Quantity`、`DerivedUnit`、`DerivedQuantity`、`PrefixMode` 等。
