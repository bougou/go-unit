...
|------|----------|------------|----------|------|
| 长度 | `LengthUnit` | `LengthQuantity` | `Length(v, u)` | `LengthQuantityParse(s)` |
| 质量 | `MassUnit` | `MassQuantity` | `Mass(v, u)` | `MassQuantityParse(s)` |
| 时间 | `TimeUnit` | `TimeQuantity` | `Time(v, u)` | `TimeQuantityParse(s)` |
| 电流 | `CurrentUnit` | `CurrentQuantity` | `Current(v, u)` | `CurrentQuantityParse(s)` |
| 温度 | `TemperatureUnit` | `TemperatureQuantity` | `Temperature(v, u)` | `TemperatureQuantityParse(s)` |
| 物质的量 | `AmountUnit` | `AmountQuantity` | `Amount(v, u)` | `AmountQuantityParse(s)` |
| 发光强度 | `LuminousUnit` | `LuminousQuantity` | `Luminous(v, u)` | `LuminousQuantityParse(s)` |
...
```go
speed := u.Length(10, u.Kilometer).Div(u.Time(2, u.Hour))
