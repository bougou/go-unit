...
> In dimensional analysis literature, thermodynamic temperature uses **Θ** (theta). The derived-dimension struct field is named `H` for historical compatibility inside the codebase.
>
## Derived dimensions
...
Derived dimensions combine the way quantities multiply and divide:

```go
d1 := u.DimForce                    // M¹L¹T⁻²
d2 := u.DimSpeed                    // L¹T⁻¹
product := d1.Add(d2)                  // multiply quantities → add exponents
quotient := d1.Sub(d2)                 // divide quantities → subtract exponents
equal := d1.Equal(u.DimForce)
```

**Addition and subtraction** of quantities require **equal** derived dimensions. Multiplying or dividing changes the derived dimension according to `Add` / `Sub`.
