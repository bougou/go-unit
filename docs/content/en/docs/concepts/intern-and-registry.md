...
```go
key := u.Unit(u.ForceUnit.Key())
du, ok := key.DerivedUnit()
```

## Named vs unnamed registration

When the **first** registrant for a composition is **named** (e.g. `ForceUnit` as `"N"`), the bare composition key aliases to that instance. Later unnamed `Intern()` calls for the same composition return the named canonical unit.

```go
// After init registers NewtonUnit first:
...
Compare derived units with `==` only after interning (or when using package globals like `ForceUnit`).
